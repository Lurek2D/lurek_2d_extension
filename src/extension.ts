...
import { TileMapEditorProvider } from "./tilemap/TileMapEditorProvider.js";
...
window.addEventListener('resize',draw);
  ...
  // ─── Custom Editors ──────────────────────────────────────
  context.subscriptions.push(TileMapEditorProvider.register(context));
}
...
const BUNDLED_LUACATS_BASENAME = "lurek.luacats";

function normalizeLibraryPath(filePath: string): string {
  return path.normalize(filePath).replace(/[\\/]+$/, "").toLowerCase();
}

function sameLibraryPaths(left: string[], right: string[]): boolean {
...
function ensureBundledAnnotationsDir(
  context: vscode.ExtensionContext,
): string | undefined {
  ...
  const targetDir = path.join(context.globalStorageUri.fsPath, "luacats");
  const targetFile = path.join(targetDir, "lurek.lua");
  fs.mkdirSync(targetDir, { recursive: true });

  const shouldRefresh =
    !fs.existsSync(targetFile)
    || fs.statSync(targetFile).size !== fs.statSync(bundledSource).size
    || fs.statSync(targetFile).mtimeMs < fs.statSync(bundledSource).mtimeMs;
...
function configureLuaWorkspaceLibrary(context: vscode.ExtensionContext): void {
  ...
  // fallback to a materialized copy outside the workspace.
  const wsRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
  ...
  const managedLibraryPaths = new Set<string>([
    wsDocsApi,
    bundledData,
    materializedBundledDir,
    wsRoot ? path.join(wsRoot, "extensions", "vscode", "data") : undefined,
  ].filter((entry): entry is string => Boolean(entry)).map(normalizeLibraryPath));
