 ...
 * Runs 14 diagnostic rules on document open, save, and change (debounced).
 */
export function register(context: vscode.ExtensionContext, apiData: ApiDataService): void {
    ...
    const diagnose = (document: vscode.TextDocument): void => {
        ...
        try {
            ...
            diagnostics.push(...checkMissingLoadCallback(text, document));
...
function checkPerFrameAllocation(
    ...
    const diagnostics: vscode.Diagnostic[] = [];
    const lines = text.split('\n');
    const allocPattern = /lurek\.(?:render\.(?:newImage|newFont|newCanvas|newShader)|audio\.(?:newSource)|physics\.(?:newBody)|image\.load)\s*\(/g;
...
function checkEntityNilAccess(text: string): vscode.Diagnostic[] {
    ...
    // Detect pattern: local X = lurek.ecs.find(...) or lurek.entity.find(...) \n X:method() or X.method without if X then
    const findPattern = /\blocal\s+(\w+)\s*=\s*lurek\.(?:ecs|entity)\.find\s*\(/g;
...
// ── Rule 14: Missing lurek.load() in main.lua ────────────────

/**
 * Warns when main.lua uses lurek.* APIs but has no lurek.load callback defined.
 * lurek.load() is the standard place for initialization logic.
 */
function checkMissingLoadCallback(
    text: string,
    document: vscode.TextDocument,
): vscode.Diagnostic[] {
    const diagnostics: vscode.Diagnostic[] = [];
    ...
    // Check if file uses lurek.* calls (not just require or comments)
    const lurekUsagePattern = /(?<!--.*)\blurek\.(?!load\b)\w+/;
    if (!lurekUsagePattern.test(text)) return diagnostics;

    // Check if lurek.load is defined
    const hasLoad = /function\s+lurek\.load\s*\(/.test(text)
        || /lurek\.load\s*=\s*function/.test(text);
    if (hasLoad) return diagnostics;

    const lines = text.split('\n');
    const range = new vscode.Range(0, 0, 0, lines[0]?.length ?? 0);
    const diag = new vscode.Diagnostic(
        range,
        'main.lua uses lurek.* APIs but does not define lurek.load(). Consider adding initialization logic in lurek.load().',
        vscode.DiagnosticSeverity.Information,
    );
    diag.code = 'lurek.missingLoad';
    diag.source = 'Lurek2D Toolkit';
    diagnostics.push(diag);
